...
function MemberProfile() {
    ...
    const fetchData = () => {
        ...
        axios.get(`http://localhost:8081/member-details/${id}?t=${Date.now()}`, { headers: { Authorization: token } })
            .then(res => {
                        ...
                        axios.put(`http://localhost:8081/update-member-auth/${id}`, authForm, { headers: { Authorization: token } })
                            .then(authRes => {
                                if (authRes.data.Status === "Success") {
                                    alert("Updated Successfully"); setIsEditing(false); fetchData();
                                } else alert("Profile updated, Auth failed: " + authRes.data.Error);
                            });
                    } else { alert("Updated Successfully"); setIsEditing(false); fetchData(); }
    ...
    const handleCancelLoan = () => {
        if(!window.confirm("Are you sure you want to CANCEL this loan? This will be recorded in history.")) return;
        const token = localStorage.getItem('token');
        axios.delete(`http://localhost:8081/cancel-loan/${data.activeLoan.id}`, { headers: { Authorization: token } })
            .then(res => {
                if(res.data.Status === "Success") { alert("Loan Cancelled"); fetchData(); }
                else alert(res.data.Error);
            });
    };
    ...
    const handleAssignRecord = (e) => {
        ...
        const selectedType = paymentForm.type;
        const activeLoanId = data?.activeLoan?.id;
        const loanIdToSend = (selectedType === 'loan_payment' && activeLoanId) ? activeLoanId : null;
    ...
    // If you want this to MARK as cancelled (instead of deleting), you might need a new endpoint.
    // For now, if the backend deletes it, it vanishes. 
    // If the backend SUPPORTS marking as 'cancelled' on delete, this works.
    const handleCancelTransaction = (recordId) => {
        if(!window.confirm("CANCEL transaction?")) return;
        const token = localStorage.getItem('token');
        axios.delete(`http://localhost:8081/delete-record/${recordId}`, { headers: { Authorization: token } }).then(res => { if(res.data.Status === "Success") fetchData(); });
    };
    ...
    const handleUndoCashOut = (type) => {
        if(!window.confirm(`Undo cashout for ${type}?`)) return;
        const token = localStorage.getItem('token');
        axios.put('http://localhost:8081/undo-cash-out', { user_id: id, type }, { headers: { Authorization: token } }).then(res => { if(res.data.Status === "Success") fetchData(); });
    };
                ...
                <div className='flex-1 w-full'>
                    {!isEditing ? (
                        ...
                        <div className='space-y-3 bg-white/10 p-4 rounded-[20px] border border-white/20'>
                            ...
                            <h3 className='text-white font-bold border-b border-white/20 pb-2 pt-2'>Login Credentials</h3>
                            <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                                <div><label className='text-xs font-bold text-white uppercase'>New Phone</label><input type="text" name="new_phone" value={authForm.new_phone} onChange={handleAuthChange} className='w-full p-2 rounded-[10px] bg-white/80 text-gray-800' /></div>
                                <div><label className='text-xs font-bold text-white uppercase'>New Password</label><input type="password" name="new_password" value={authForm.new_password} onChange={handleAuthChange} className='w-full p-2 rounded-[10px] bg-white/80 text-gray-800' /></div>
                            </div>
                            <div className='flex gap-2 mt-4'><button onClick={handleSaveMember} className='bg-green-600 text-white px-4 py-2 rounded-[10px] hover:bg-green-700 font-semibold'>Save</button><button onClick={() => setIsEditing(false)} className='bg-gray-500 text-white px-4 py-2 rounded-[10px] hover:bg-gray-600'>Cancel</button></div>
                        </div>
                    )}
                </div>
            </div>
            ...
            <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
                    ...
                    <div className='bg-white/0 backdrop-blur-[50px] p-6 rounded-[30px] shadow border border-white/50'>
                        ...
                        {!data.activeLoan ? (
                            <div className='flex flex-col gap-3'>
                                <input type="text" placeholder="Loan Name" className='border p-2 rounded-[15px] w-full outline-none focus:ring-2 focus:ring-blue-500' value={loanName} onChange={e => setLoanName(e.target.value)} />
                                <div className='flex gap-2'>
                                    <input type="number" placeholder="Enter Amount" className='border p-2 rounded-[15px] w-full outline-none focus:ring-2 focus:ring-blue-500' value={loanAmount} onChange={e => setLoanAmount(e.target.value)} />
                            ...
                            <div className='text-center'>
                                <div className='flex justify-between items-center mb-2'>
                                    <h3 className='text-white font-bold text-lg'>{data.activeLoan.loan_name || 'Active Loan'}</h3>
                                    <button onClick={handleCancelLoan} className='text-xs bg-red-500/20 text-red-300 px-3 py-1 rounded-full hover:bg-red-500/40 border border-red-500/30 transition'>Cancel Loan</button>
                                </div>
                    ...
                    <div className='bg-white/0 backdrop-blur-[50px] p-6 rounded-[30px] shadow border border-white/50'>
                        ...
                        <form onSubmit={handleAssignRecord} className='space-y-3'>
                            <div>
                                ...
                                <select 
                                    className='w-full border p-2 rounded-[10px] bg-white/80 text-gray-800' 
                            ...
                            <div>
                                ...
                                <input type="number" placeholder="0.00" className='w-full border p-2 rounded-[10px] bg-white/80 text-gray-800' required value={paymentForm.amount} onChange={e => setPaymentForm({...paymentForm, amount: e.target.value})} />
                            </div>
                            <div>
                                <label className='text-xs font-bold text-white uppercase'>Due</label>
                ...
                <div className='space-y-6'>
                    <div className='grid grid-cols-2 gap-4'>
                        <div className='bg-white/0 backdrop-blur-[50px] p-4 rounded-[30px] shadow border border-white/50 text-center relative group'>
                            ...
                            {Number(data.savingsTotal) > 0 ? (
                                <button onClick={() => handleCashOut('savings')} className='mt-2 text-xs bg-purple-100 text-purple-700 px-3 py-1 rounded hover:bg-purple-200 font-bold'>Cash Out</button>
                            ) : (
                                <button onClick={() => handleUndoCashOut('savings')} className='mt-2 text-xs bg-white/10 text-white px-3 py-1 rounded hover:bg-white/20 border border-white/20'>Undo Cashout</button>
                            )}
                        </div>
                        <div className='bg-white/0 backdrop-blur-[50px] p-4 rounded-[30px] shadow border border-white/50 text-center'>
                            ...
                            {Number(data.insuranceTotal) > 0 ? (
                                <button onClick={() => handleCashOut('insurance')} className='mt-2 text-xs bg-orange-100 text-orange-700 px-3 py-1 rounded hover:bg-orange-200 font-bold'>Cash Out</button>
                            ) : (
                                <button onClick={() => handleUndoCashOut('insurance')} className='mt-2 text-xs bg-white/10 text-white px-3 py-1 rounded hover:bg-white/20 border border-white/20'>Undo Cashout</button>
                            )}
                        </div>
                    </div>
                                    ...
                                    {filteredRecords.length > 0 ? filteredRecords.map((rec, index) => (
                                        <tr key={index} className='hover:bg-white/10 transition'>
                                            <td className='p-3 capitalize text-white'>
                                                {rec.type.startsWith('Cancelled:') ? <span className="text-red-300">{rec.type}</span> :
                                                 rec.type === 'loan_payment' && rec.loan_name ? <span>Loan Pmt: {rec.loan_name}</span> : 
                                                 rec.type.replace('_', ' ')}
                                            </td>
                                            <td className='p-3 font-medium text-white'>₱{rec.amount}</td>
                                            <td className='p-3 text-white'>{rec.due_date ? new Date(rec.due_date).toLocaleDateString() : 'No Date'}</td>
                                            <td className='p-3'>
                                                <span className={`px-2 py-1 rounded-full text-xs font-bold uppercase ${
                                                    rec.status === 'paid' ? 'bg-green-100 text-green-700' : 
                                                    rec.status === 'late' ? 'bg-orange-100 text-orange-800' : 
                                                    rec.status === 'pending' ? 'bg-blue-100 text-blue-700' :
                                                    rec.status === 'cancelled' ? 'bg-red-600 text-white' : // FIXED: Explicit Cancelled Style
                                                    rec.status === 'cashed_out' ? 'bg-gray-400 text-gray-200 line-through' :
                                            ...
                                            <td className='p-3 flex justify-center items-center gap-2'>
                                                {rec.status === 'pending' && (
                                                    <button 
                                                        ...
                                                        className='bg-green-500/20 text-green-400 border border-green-500/50 px-3 py-1 rounded-full text-xs hover:bg-green-500 hover:text-white transition font-bold shadow-sm'
                                                    >
                                                        ✓ Pay
                                                    </button>
                                                )}
                                                ...
                                                {rec.status !== 'cancelled' && <button onClick={() => handleCancelTransaction(rec.id)} className='text-red-400 hover:text-red-600 font-bold ml-2' title="Cancel Transaction">✕</button>}
                                            </td>
                                        </tr>
                                    )) : <tr><td colSpan="5" className='p-6 text-center text-white italic'>No records found</td></tr>}
                                </tbody>
                            </table>
                        </div>
                    </div>
