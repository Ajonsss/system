...
function Dashboard() {
    ...
    // These were missing in your snippet and are required for the page to work.
    const [role, setRole] = useState('');
    const [myData, setMyData] = useState(null); // Initialize as null to check loading state
    const [records, setRecords] = useState([]);
    const [notifications, setNotifications] = useState([]); // Initialized as [] to fix the .filter error
    ...
    useEffect(() => {
        ...
        axios.get(`http://localhost:8081/member-details/${storedId}`, { headers: { Authorization: token } })
            .then(res => {
                if(res.data.user) {
                    setMyData(res.data);
                    setRecords(res.data.records || []); // Safety check
                    setNotifications(res.data.notifications || []); // Safety check
                }
            })
    ...
    // --- 2. Fixed the Crash Here ---
    // Added optional chaining (?.) and a safety check.
    // This will now default to 0 if notifications is somehow undefined.
    ...
    // If data hasn't loaded yet, show a loading text instead of crashing when trying to read 'myData.activeLoan'
                    ...
                    <div className='flex items-center gap-4'>
                        <div className='relative'>
                            <div className='cursor-pointer p-2 hover:bg-white/10 rounded-full transition' onClick={() => setShowNotif(!showNotif)}>
                                ...
                                {unreadCount > 0 && (<span className='absolute top-0 right-0 bg-red-500 text-white text-[10px] px-1.5 py-0.5 rounded-full border border-white/20 shadow-md'>{unreadCount}</span>)}
                            </div>
                            {showNotif && (
                        ...
                        <div className='flex flex-col gap-6'>
                            ...
                            <div className='grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-white/20 pt-6'>
                                <div className='bg-white/10 backdrop-blur-[10px] p-4 rounded-[20px] border border-white/30'>
                                    ...
                                    {myData.activeLoan ? <div><div className='flex justify-between items-end mb-2'><span className='text-3xl font-bold text-white'>{Math.round(loanProgress)}%</span><span className='text-sm text-white/80 font-medium'>₱{myData.activeLoan.current_balance} left</span></div><div className='w-full bg-white/20 rounded-full h-2.5 overflow-hidden'><div className='bg-blue-500 h-2.5 rounded-full transition-all duration-500' style={{ width: `${loanProgress}%` }}></div></div></div> : <p className='text-white/50 italic'>No active loan</p>}
                                </div>
                                <div className='bg-white/10 backdrop-blur-[10px] p-4 rounded-[20px] border border-white/30'><h3 className='text-white font-bold text-sm uppercase mb-1'>Total Savings</h3><p className='text-3xl font-bold text-white'>₱{myData.savingsTotal}</p></div>
                                <div className='bg-white/10 backdrop-blur-[10px] p-4 rounded-[20px] border border-white/30'><h3 className='text-white font-bold text-sm uppercase mb-1'>Total Insurance</h3><p className='text-3xl font-bold text-white'>₱{myData.insuranceTotal}</p></div>
                            </div>
                        </div>
                    )}
                </div>
                                ...
                                return (
                                    <div key={index} onClick={() => navigate(`/member/${member.id}`)} className='membercard'>
                                        <div className='flex items-center gap-4'>
                                            ...
                                            <div className='flex-1'><p className='text-white'>{member.full_name}</p><p className='text-xs text-white'>{member.phone_number}</p><p className={`text-xs mt-1 ${member.late_count > 0 ? 'text-red-400' : 'text-white'}`}>Late Payments: {member.late_count}</p></div>
                                ...
                                <tbody className='divide-y divide-gray-100'>
                                    {filteredRecords.length > 0 ? filteredRecords.map((rec, i) => (
                                        <tr key={i} className='hover:bg-white/20 transition'>
                                            ...
                                            <td className=' text-white px-6 py-4'><span className={`px-2 py-1 rounded-full text-xs font-bold ${rec.status === 'paid' ? 'bg-green-100 text-green-700' : rec.status === 'late' ? 'bg-orange-100 text-orange-700' : rec.status === 'cancelled' ? 'bg-red-900/50 text-red-200' : rec.status === 'cashed_out' ? 'bg-gray-200 text-gray-500 line-through' : 'bg-yellow-100 text-yellow-700'}`}>{rec.status.replace('_', ' ')}</span></td>
